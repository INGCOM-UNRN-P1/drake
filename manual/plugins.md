---
title: "Guía de Extensión y Creación de Plugins: drake"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de drake"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-drake-plugins)=
# Guía de Extensión y Plugins: drake

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`drake`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-drake-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`drake` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para drake`.
- **Punto de Entrada Oficial**: `drake.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-drake-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para drake."""
    name = "custom_rule"
...
Para que `drake` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."drake.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `drake` la reconozca:
...
(manual-drake-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `drake` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = drake.ejecutar_analisis(
...
(manual-drake-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `drake` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar drake
        run: pip install -e ./drake
        
      - name: Ejecutar Auditoría
        run: drake check src/ include/ --json > reporte.json
````

---

(manual-drake-plugins-ejercicios)=
